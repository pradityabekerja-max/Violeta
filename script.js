...
function initBirthdayApp(){
  ...
  const targetAttr = countdownContainer.dataset && countdownContainer.dataset.target;
...
function updateCountdown(){
  ...
  try{
    ...
    if(params.get('dev') === '1' || location.hostname === 'localhost' || location.hostname === '127.0.0.1'){
      clearInterval(countdownInterval);
      showForm();
    }
  } catch(e){/* ignore when not in browser environment */}
...
function showFormStep(step){
  ...
  function renderStep(){
    steps.forEach(s=>{
      ...
      if(isCurrent){
        const focusable = s.querySelector('input, textarea, button, select, label');
  ...
  if(nextBtnEl){
    nextBtnEl.addEventListener('click', ()=>{
      if(!validateStep(currentStep)){
        if(responseDiv) responseDiv.textContent = 'Mohon isi jawaban sebelum melanjutkan.';
        return;
      }
...
function tryPlayMusic(){
  ...
  if(playPromise !== undefined){
    playPromise.catch(()=>{/* autoplay blocked; we'll try on user gesture */});
  }
}
...
// Form submission
  if(form){
    ...
    const useMock = isLocal || urlParams.get('mock') === '1';
    ...
    form.addEventListener('submit', async (e)=>{
      ...
      if(!validateStep(lastStep)){
        if(responseDiv) responseDiv.textContent = 'Mohon isi jawaban terakhir sebelum mengirim.';
        return;
      }
      ...
      if(useMock){
        ...
        if(responseDiv) responseDiv.textContent = 'Terima kasih (mock): jawaban disimpan (dev).';
        form.reset();
      ...
      try{
        ...
        if(!res.ok){
          ...
          console.error(msg);
          if(responseDiv) responseDiv.textContent = `Gagal mengirim: ${res.status} ${res.statusText}. Cek console.`;
          return;
        }

        // Success
        let resultText = '';
        try{ resultText = await res.text(); } catch(e){}
        if(responseDiv) responseDiv.textContent = 'Terima kasih, jawaban Anda telah tersimpan!';
        form.reset();
        form.classList.add('hidden');
        form.setAttribute('aria-hidden','true');
        if(finalContainer) finalContainer.classList.remove('hidden');
        ...
        if(responseDiv) responseDiv.innerHTML = `Terjadi kesalahan jaringan saat mengirim: ${err.message}.<br>Kamu bisa: <strong>1)</strong> jalankan dengan ?mock=1 atau di localhost untuk mengetes, atau <strong>2)</strong> periksa koneksi/CORS endpoint.`;

        // If running locally, fallback to mock success to allow testing without changing URL
        if(isLocal){
          console.info('Falling back to mock submission because we are on localhost');
          await new Promise(r=>setTimeout(r,500));
          if(responseDiv) responseDiv.textContent = 'Terima kasih (fallback mock): jawaban disimpan (dev).';
          form.reset();
          form.classList.add('hidden');
          form.setAttribute('aria-hidden','true');
          if(finalContainer) finalContainer.classList.remove('hidden');
